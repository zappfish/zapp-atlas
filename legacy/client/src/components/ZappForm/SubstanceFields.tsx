...
export default function SubstanceFields({
  ...
  const [infoRevealed, setInfoRevealed] = useState(() => !!value.name);
  ...
  useEffect(() => {
    ...
    if (!option?.meaning) { setNorm({ status: 'idle' }); return; }
    const colon = option.meaning.indexOf(':');
    const ns = option.meaning.slice(0, colon);
    const id = option.meaning.slice(colon + 1);
    let cancelled = false;
    ...
    fetch('/normalize-chemical', {
      ...
      .then((data) => {
        if (cancelled) return;
        const result: NormResult = data.result;
        if (result?.normalized)
          setNorm({ status: 'done', items: [{ queried_id: option.meaning!, result, imageB64: data.structure_image_b64 ?? null }], mode: 'id' });
        else
          setNorm({ status: 'error', message: 'No NodeNorm entry found for this vehicle.' });
      })
  ...
  const normFetch = async (body: object, queriedId: string): Promise<NormResultItem | null> => {
    try {
      const res = await fetch('/normalize-chemical', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  ...
  const handleSuggestionClick = async (suggestion: AcSuggestion) => {
    ...
    if (suggestion.normalized?.normalized) {
      setNorm({ status: 'done', items: [{ queried_id: suggestion.chebi_ids[0] ?? suggestion.name, result: suggestion.normalized, imageB64: null }], mode: 'name' });
    } else {
      setNorm({ status: 'loading' });
  ...
  return (
    <div className="col-12" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {/* ── 2. Results (always present, fixed height, scrollable) ── */}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                <Input
                  ...
                  value={value.name || ''}
                  disabled={!!value.unrecognized_name}
                ...
                <Input
                  ...
                  value={value.unrecognized_name || ''}
                  disabled={!!value.name}
                  onChange={(e) => applyChange({ unrecognized_name: e.target.value })}
                />
                <Input
                  label="CAS Number"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    applyChange({ manufacturer: v === '__other__' ? '' : v });
                  }}
                />
                {(mfrSelectVal === '__other__' || isOtherMfr) && (
                  <Input
                    label="Manufacturer (specify)"
      ...
      {/* Vehicle mode: just standardized ID below */}
      {mode === 'vehicle' && (
        <div style={SECTION_BOX}>
          <div style={SECTION_LABEL}>Standardized information</div>
