...
def test_add_chemical_appears_in_the_cabinet(client: TestClient) -> None:
    _sign_in(client)
    ...
    assert "CHEBI:16236" in page.text


def test_add_chemical_over_htmx_redirects(client: TestClient) -> None:
    _sign_in(client)
    group_path = _new_group(client)

    res = client.post(
        f"{group_path}/chemical-cabinet",
        data={"chemical_id": "CHEBI:35456"},
...
def test_non_member_cannot_add_chemical(client: TestClient) -> None:
